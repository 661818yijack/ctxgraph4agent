# ctxgraph4agent — Round 2 User Stories (MiniMax M2.7 Review Applied)
...
> A3 gains renewal_count field, B3 compression no longer runs every query, C2 uses renewal_count
...
## A3: Add usage_count, renewal_count, and last_recalled_at tracking
...
### Description
Add `usage_count: u32`, `renewal_count: u32`, and `last_recalled_at: Option<DateTime<Utc>>` to Entity and Edge. **These are two separate counters** (review fix): `usage_count` tracks how often a memory appears in retrieval results (for A4b scoring bonus), while `renewal_count` tracks how many times its TTL was renewed (for C2 renewal limiting). They are updated when memories appear in search/retrieval results that are consumed by the agent. A new `touch_memory` method increments usage_count and sets last_recalled_at to now. Renewal_count is only incremented by `renew_memory` (C2). These fields feed into re-verification (Phase C) and budget ranking (A4b).

### Acceptance Criteria
1. `Entity` and `Edge` structs have `usage_count: u32` (default 0), `renewal_count: u32` (default 0), and `last_recalled_at: Option<DateTime<Utc>>` (default None)
2. Migration 004 adds `usage_count INTEGER NOT NULL DEFAULT 0`, `renewal_count INTEGER NOT NULL DEFAULT 0`, and `last_recalled_at TEXT` columns to `entities` and `edges` tables
...
5. `Storage::touch_many(ids: &[(MemoryTable, &str)])` batch-touches multiple memories in a single transaction (for efficiency in retrieve_for_context)
6. `Storage::get_usage_stats()` returns total counts grouped by memory_type for dashboard/monitoring (only active records, not expired)
7. All read paths (get_entity, list_entities, get_edge, etc.) populate the new fields
...
- **Files to create/modify:**
  ...
  - `crates/ctxgraph-core/src/storage/sqlite.rs` — add `touch_entity`, `touch_edge`, `touch_many`, `get_usage_stats`; update all read paths
- **New types/functions:**
  - `Storage::touch_entity(&self, id: &str) -> Result<()>`
  - `Storage::touch_edge(&self, id: &str) -> Result<()>`
  - `Storage::touch_many(&self, items: &[(MemoryTable, &str)]) -> Result<()>`
  - `Storage::get_usage_stats(&self) -> Result<Vec<(MemoryType, u64, u64)>>`
- **Indexes:** Add index on `(memory_type, usage_count)` for stats queries; add index on `renewal_count` for renewal limit checks
- **Config changes:** none

### Test Plan
- Integration: insert entity, call `touch_entity` 3 times, read entity back, verify `usage_count == 3` and `renewal_count == 0`
- Integration: call `touch_entity`, verify `last_recalled_at` is set to recent timestamp
- Integration: `touch_many` on 5 entities updates all 5 in one transaction
- Integration: migration 004 applied to existing DB, existing rows get `usage_count=0`, `renewal_count=0`, `last_recalled_at=NULL`
- Unit: `touch_entity` on nonexistent id returns error
- Unit: `get_usage_stats` only counts active records (not those with decay_score = 0)
...
- Integration: deduplication — same entity from FTS5 and graph traversal appears once with higher score
...
### Description
Implement the scoring and ranking step that computes a composite score for each retrieval candidate. This is the second of three stories that replace the original A4. The composite score uses decay_score (A2) for freshness, FTS5 BM25 for relevance, and usage_count (A3) for a recency bonus. **Critical fix:** the scoring bonus uses `usage_count` only (how often recalled), NOT `renewal_count` — these are separate fields added in A3. After scoring, candidates are sorted descending by composite score. Patterns always get a minimum score of 0.5 (floor) to ensure they surface unless budget is truly exhausted.

### Acceptance Criteria
1. `score_candidate(candidate: &RetrievalCandidate, base_confidence: f64) -> f64` computes composite score
2. Composite score = `decay_score * normalized_fts_score * (1.0 + 0.1 * ln(1 + usage_count))` — usage_count is the recall frequency, NOT renewal_count
...
6. `Storage::retrieve_for_context(query, policy, budget_tokens)` orchestrates A4a + A4b + A4c in one call
7. `RankedMemory` struct includes `token_budget_spent` field

### Technical Requirements
- **Files to create/modify:**
  - `crates/ctxgraph-core/src/types.rs` — add `RankedMemory` struct (with token_budget_spent), `MemoryPolicy` struct, `estimate_tokens` function
...
- **New types/functions:**
  ...
  - `MemoryPolicy { memory_budget_tokens: usize, agent_name: String, max_patterns_included: usize }`
  - `MemoryPolicy::default() -> MemoryPolicy` (budget = 20_000, max_patterns = 50)
...
### Acceptance Criteria
1. `ctxgraph.toml` supports `[policies.<name>]` with keys: `facts_ttl`, `experiences_ttl`, `patterns_ttl` (always "never"), `preferences_ttl`, `decisions_ttl`, `memory_budget_tokens`, `compress_after`, `max_episodes`, `max_patterns_included` (default 50), `stale_threshold` (default 0.3)
...
- **Files to create/modify:**
  ...
  - `crates/ctxgraph-core/src/storage/migrations.rs` — add migration 008 (archived_entities, archived_edges, system_metadata tables)
...
8. Uniqueness constraint on `(source_id, target_id, relation, compression_id)` in `compression_groups` prevents duplicate insertions
...
4. `PatternCandidate { entity_types: Vec<String>, entity_pair: Option<(String, String)>, relation_triplet: Option<(String, String, String)>, occurrence_count: u32, source_groups: Vec<String> }`
...
7. New `learn` MCP tool runs full pipeline: extract patterns from recent compressions, create/update skills
...
│       └── D2 (skill creation, configurable relations) ── depends on D1b
...
| 005       | B1    | Add `compression_groups` table, `compression_id` to episodes, `compressed_at` field |
| 006       | D2    | Add `skills` table + FTS5 index on description |
| 007       | D3    | Add `skill_sources` table, `scope` and `created_by_agent` columns to skills |
| 008       | A6    | Add `archived_entities`, `archived_edges`, `system_metadata` tables (for TTL cleanup); add index on `(memory_type, created_at)` |
